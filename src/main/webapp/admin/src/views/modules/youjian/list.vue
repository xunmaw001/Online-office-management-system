            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '邮件标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.youjianName" placeholder="邮件标题" clearable></el-input>
                     </el-form-item>
                 
                     <el-form-item :label="contents.inputTitle == 1 ? '邮件类型' : ''">
                        <el-select v-model="searchForm.youjianTypes" placeholder="请选择邮件类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in youjianTypesSelectSearch"
                               v-bind:key="index"
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '接收人' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.youjianJieshouren" placeholder="接收人" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('youjian','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xianshangbangongguanlixitong/upload/youjianMuBan.xls"
                        >批量导入邮件数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('youjian','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="youjianUploadSuccess"
                                :on-error="youjianUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('youjian','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "youjian.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="邮件标题">
                        <template slot-scope="scope">
                            {{scope.row.youjianName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="youjianFile"
                                     header-align="center"
                                     width="200"
                                     label="邮件附件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.youjianFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="scope.row.youjianFile"  >下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="youjianJieshouren"
                                   header-align="center"
                                   label="接收人">
                        <template slot-scope="scope">
                            {{scope.row.youjianJieshouren}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('youjian','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('youjian','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('youjian','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'youjian',//当前表表名,
                        sumColum : 'youjian_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'youjian_name',//分组字符串字段
                        // types : 'youjian_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.youjianName!= '' && this.searchForm.youjianName!= undefined) {
                    params['youjianName'] = '%' + this.searchForm.youjianName + '%'
                }
                 
                if (this.searchForm.youjianTypes!= '' && this.searchForm.youjianTypes!= undefined) {
                    params['youjianTypes'] = this.searchForm.youjianTypes
                }
                         
                if (this.searchForm.youjianJieshouren!= '' && this.searchForm.youjianJieshouren!= undefined) {
                    params['youjianJieshouren'] = '%' + this.searchForm.youjianJieshouren + '%'
                }
        
                params['youjianDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=youjian_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.youjianTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            youjianUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "youjian/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
